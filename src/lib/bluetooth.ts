...
export async function transmitSecurityCode(
  ...
  try {
    ...
    connectedDevice = await device.connect({ timeout: 15000, requestMTU: 256 });
    ...
    if (!serviceExists) {
      await disconnectDevice();
      return { success: false, error: 'Service-ul ESP32 nu a fost găsit. Verifică codul ArduinoBLE.' };
    }
